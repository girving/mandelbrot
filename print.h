...
template<class... Args> void print(const char* fmt, const Args&... args) { print(tfm::format(fmt, args...)); }

template<class T> static inline void print_error(const T& x) { print_error(str(x)); }
template<class... Args> void print_error(const char* fmt, const Args&... args) { print_error(tfm::format(fmt, args...)); }
